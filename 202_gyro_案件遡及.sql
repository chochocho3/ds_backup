...
update events set master_event_status_id = , lock_version = lock_version + 1, updated_at = now() where id = '';
--02_案件完了日時
update events set event_completed_time = '', lock_version = lock_version + 1, updated_at = now() where id = '';
="update events set event_completed_time = '"&TEXT(B2,"yyyy-MM-dd")&" 19:00:00', lock_version = lock_version + 1, updated_at = now() where id = '"&A2&"';" --excel用
--03_案件顧客リンクid
update events set customer_link_id = , lock_version = lock_version + 1, updated_at = now() where id = '';
...
=" ("&D2&", '"&TEXT(C2,"yyyy-MM-dd")&"', 9, 981, true, now(), now()),"
